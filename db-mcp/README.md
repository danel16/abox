...
3. **Add new tools**:
   ```bash
   kmcp add-tool my-tool --description "My custom tool"
   ```
...
1. **Build the Docker image**:
   ```bash
   docker build -t db-mcp:0.1.0 .
   ```

2. **Run the container**:
   ```bash
   # Stdio mode
   docker run -it db-mcp:0.1.0
   
   # HTTP mode
   docker run -p 3000:3000 db-mcp:0.1.0 --transport http
   ```
...
3. Be registered in the `Tools.java` file (automatically handled by `kmcp add-tool`)
...
- **kmcp.yaml**: Project configuration file
...
```bash
kmcp deploy --namespace production
```
...
For support and questions, please refer to the [KMCP documentation](https://kagent.dev/docs/kmcp).
